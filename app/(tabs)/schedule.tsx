...
function PendingCard({ item }: { item: import("@/lib/notification-store").NotificationRecord }) {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      Animated.timing(feedbackProgress, { toValue: 1, duration: 220, easing: Easing.out(Easing.cubic), useNativeDriver: true }).start();
      setTimeout(() => {
        Animated.timing(feedbackProgress, { toValue: 0, duration: 180, easing: Easing.in(Easing.cubic), useNativeDriver: true }).start(() => setEditFeedback(false));
      }, 1800);
    } catch (error) {
  ...
  return <>
    <Animated.View style={[styles.pending, { opacity: cardProgress, transform: [{ translateY: cardProgress.interpolate({ inputRange: [0, 1], outputRange: [10, 0] }) }] }]}>
      ...
      {editFeedback ? <Animated.View style={[styles.editFeedback, { opacity: feedbackProgress, transform: [{ scale: feedbackProgress.interpolate({ inputRange: [0, 1], outputRange: [0.92, 1] }) }] }]}><IconSymbol name="checkmark.circle.fill" size={17} color="#2C8D69" /><Text style={styles.editFeedbackText}>Agendamento atualizado</Text></Animated.View> : null}
      <View style={styles.actions}><Pressable onPress={openEditor} style={({ pressed }) => [styles.secondary, pressed && styles.pressed]}><IconSymbol name="square.and.pencil" size={17} color={teal} /><Text style={styles.secondaryText}>Editar</Text></Pressable><Pressable onPress={() => void cancel(item)} style={({ pressed }) => [styles.secondary, pressed && styles.pressed]}><Text style={styles.secondaryText}>Cancelar</Text></Pressable><Pressable onPress={() => void remove(item)} style={({ pressed }) => [styles.delete, pressed && styles.pressed]}><IconSymbol name="trash" size={19} color="#B44B47" /><Text style={{ color: "#B44B47", fontWeight: "800" }}>Excluir</Text></Pressable></View>
    </Animated.View>

...
import { AccountFinalInput } from "@/components/account-final-input";
import { formatAccountFinal } from "@/lib/pix-account";
...
export default function ScheduleScreen() {
  ...
  const [accountNumber, setAccountNumber] = useState("15448");
  const [accountDigit, setAccountDigit] = useState("3");
    ...
    <ScreenContainer containerClassName="bg-[#EAF4F8]">
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <QuickPresetCard
          ...
          onAccountDigitChange={setAccountDigit}
...
function QuickPresetCard({ kind, name, value, accountNumber, accountDigit, onNameChange, onValueChange, onAccountNumberChange, onAccountDigitChange, onRandomName, onSchedule, onApply }: { kind: "received" | "sent"; name: string; value: string; accountNumber: string; accountDigit: string; onNameChange: (value: string) => void; onValueChange: (value: string) => void; onAccountNumberChange: (value: string) => void; onAccountDigitChange: (value: string) => void; onRandomName: () => void; onSchedule: () => void; onApply: () => void }) {
...
<View style={styles.actions}><Pressable onPress={openEditor} style={styles.secondary}><IconSymbol name="square.and.pencil" size={17} color={teal} /><Text style={styles.secondaryText}>Editar</Text></Pressable><Pressable onPress={() => void cancel(item)} style={styles.secondary}><Text style={styles.secondaryText}>Cancelar</Text></Pressable><Pressable onPress={() => void remove(item)} style={styles.delete}><IconSymbol name="trash" size={19} color="#B44B47" /><Text style={{ color: "#B44B47", fontWeight: "800" }}>Excluir</Text></Pressable></View></View>
    <Modal visible={editing} transparent animationType="slide" onRequestClose={() => !saving && setEditing(false)}><View style={styles.modalBackdrop}><View style={styles.modalCard}><Text style={styles.modalTitle}>Editar agendamento</Text><Text style={styles.modalDate}>{item.scheduledAt ? `Será enviado ${formatNotificationTime(item.scheduledAt)}` : ""}</Text><Field label="Título" value={editTitle} onChangeText={setEditTitle} placeholder="Título da notificação" /><Field label="Subtítulo (Opcional)" value={editSubtitle} onChangeText={setEditSubtitle} placeholder="Subtítulo da notificação" /><Field label="Mensagem" value={editBody} onChangeText={setEditBody} placeholder="Mensagem da notificação" multiline /><View style={styles.modalActions}><Pressable disabled={saving} onPress={() => setEditing(false)} style={styles.modalCancel}><Text style={styles.modalCancelText}>Cancelar</Text></Pressable><Pressable disabled={saving} onPress={() => void saveEdit()} style={[styles.modalSave, saving && { opacity: 0.65 }]}><Text style={styles.modalSaveText}>{saving ? "Salvando..." : "Salvar"}</Text></Pressable></View></View></View></Modal>
  </>;
}
